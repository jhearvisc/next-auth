...
export function TypeORMAdapter(dataSource, options) {
    ...
    const c = {
        dataSource,
        entities: {
            RoleEntity: entities?.RoleEntity ?? defaultEntities.RoleEntity,
    ...
    return {
        ...
        createUser: async (data) => {
            const m = await getManager(c);
            const _data = { ...data, emailVerified: data.emailVerified?.getTime().toString() };
            const user = await m.save(c.entities.UserEntity, _data);
            return user;
        },
        // @ts-expect-error
        async getUser(id) {
            const m = await getManager(c);
            const user = await m.findOne(c.entities.UserEntity, { where: { id } });
            if (!user)
                return null;
            return { ...user };
        },
        ...
        async getUserByEmail(email) {
            const m = await getManager(c);
            const user = await m.findOne(c.entities.UserEntity, { where: { email } });
            if (!user)
                return null;
            return { ...user };
        },
        ...
        async deleteUser(id) {
            ...
            await m.transaction(async (tm) => {
                await tm.delete(c.entities.AccountEntity, { userId: id });
                await tm.delete(c.entities.SessionEntity, { userId: id });
                await tm.delete(c.entities.UserEntity, { id });
            });
        },
        async linkAccount(data) {
            const m = await getManager(c);
            const account = await m.save(c.entities.AccountEntity, data);
            return account;
        },
        async unlinkAccount(providerAccountId) {
            const m = await getManager(c);
            await m.delete(c.entities.AccountEntity, providerAccountId);
        },
        async createSession(data) {
            const m = await getManager(c);
            const _data = { ...data, expires: data.expires?.getTime().toString() };
            const session = await m.save(c.entities.SessionEntity, _data);
            const _session = { ...session, expires: new Date(session.expires) };
            return _session;
        },
        async getSessionAndUser(sessionToken) {
            const m = await getManager(c);
            const sessionAndUser = await m.findOne(c.entities.SessionEntity, { where: { sessionToken }, relations: ["user"] });
            if (!sessionAndUser)
                return null;
        ...
        async updateSession(data) {
            const m = await getManager(c);
            const _data = { ...data, expires: data.expires?.getTime().toString() };
            await m.update(c.entities.SessionEntity, { sessionToken: data.sessionToken }, _data);
            // TODO: Try to return?
            return null;
        },
        async deleteSession(sessionToken) {
            const m = await getManager(c);
            await m.delete(c.entities.SessionEntity, { sessionToken });
        },
        async createVerificationToken(data) {
            const m = await getManager(c);
            const _data = { ...data, expires: data.expires?.getTime().toString() };
            const verificationToken = await m.save(c.entities.VerificationTokenEntity, _data);
        ...
        async useVerificationToken(identifier_token) {
            const m = await getManager(c);
            const verificationToken = await m.findOne(c.entities.VerificationTokenEntity, {
                where: identifier_token,
            });
            if (!verificationToken) {
                return null;
            }
            await m.delete(c.entities.VerificationTokenEntity, identifier_token);
            // @ts-expect-error
            delete verificationToken.id;
